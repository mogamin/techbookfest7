...
学習データセットを使った時と比べて性能は低いです。これが過学習(over fitting)という状態です。まったく同じ学習データの画像しか入力されないのであれば、over fittingで良いのですが、一般的にはそうではありません。
...
@<icon>{cheiko} 「だ、だいじょうぶよ。より理解するために今回の復習をするつもりよ。」
